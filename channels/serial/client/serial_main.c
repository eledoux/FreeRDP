...
static void serial_process_irp_write(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	if (status < 0)
	{
		...
		DEBUG_WARN("serial_tty_write failure: status: %d, errno: %d\n", status, errno);
...
{
	DEBUG_SVC("IRP MajorFunction: 0x%04X MinorFunction: 0x%04X\n", irp->MajorFunction, irp->MinorFunction);
...
int DeviceServiceEntry(PDEVICE_SERVICE_ENTRY_POINTS pEntryPoints)
{
	...
	if ((name && name[0]) && (path && path[0]))
	{
		...
		serial->path = path;
		serial->IrpQueue = MessageQueue_New();

		DEBUG_SVC("initializing");
