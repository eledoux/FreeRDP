...
BOOL serial_tty_read(SERIAL_TTY* tty, BYTE* buffer, UINT32* Length)
{
	...
	else if (tty->read_interval_timeout)
	{
		...
		ptermios = (struct termios*) malloc(sizeof(struct termios));
		ZeroMemory(ptermios, sizeof(struct termios));
...
SERIAL_TTY* serial_tty_new(const char* path, UINT32 id)
{
	...
	// Might not be required but at least don't ICRNL (issue #1044)
